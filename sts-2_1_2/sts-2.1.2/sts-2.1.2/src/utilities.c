		...
		switch( option ) {
			case 0:
				...
				*streamFile = (char*)calloc(200, sizeof(char));
				sprintf(*streamFile, "../../../Results/%s", input_file);
				printf("\n");
...
fixParameters()
{
	...
	do {
		...
		if ( testVector[TEST_NONPERIODIC] == 1 ) {
			counter++;
			if ( counter == testid ) {
				printf("   Enter NonOverlapping Template Test block Length: 160");
				//scanf("%d", &tp.nonOverlappingTemplateBlockLength);
				tp.nonOverlappingTemplateBlockLength = 160;
				testid = 0;
				printf("\n");
				continue;
			}
...
openOutputStreams(int option)
{
	...
	sprintf(summaryfn, "experiments/%s/report_%s", generatorDir[option], input_file);

...
int error_tests[NUMOFTESTS+1];
double avg_P[NUMOFTESTS+1];
int num_runs[NUMOFTESTS+1];
int avg_pass_count[NUMOFTESTS+1];
int test_sample_size[NUMOFTESTS+1];
...
main(int argc, char *argv[])
{
	...
	if ( argc != 2 ) {
		...
		printf("   100000 is the length of the individual bit stream(s) to be processed\n");
		//return 0;
	}
...
postProcessResults(int option)
{
	...
	for(i = 0; i < NUMOFTESTS ; i++){
		if(error_tests[i]){
			fprintf(summary, "FAILED: %s\n", testNames[i]);
		}
	}

	fprintf(summary, "---------------------------------------------------------------------------\n\n\n\n\n");

	fprintf(summary, "Final average P-Values For Finished Tests\n\n");
	for(int i = 0; i <= NUMOFTESTS; i++){
		if(num_runs[i] != 0){
			fprintf(summary, "%8.6f - %d/%d - %s\n", avg_P[i], avg_pass_count[i], test_sample_size[i], testNames[i]);
		}
	}
...
computeMetrics(char *s, int test)
{
	...
	uniformity = sum / sampleSize;
	
	if(validTest && test != TEST_UNIVERSAL){

		avg_P[test] *= num_runs[test];
		avg_P[test] += uniformity;
		avg_pass_count[test] += passCount;
		num_runs[test] = num_runs[test] + 1;
		avg_P[test] /= num_runs[test];
	...
	test_sample_size[test] += sampleSize;
